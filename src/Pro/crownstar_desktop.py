...
engine = CrownStarCognitive(tier=TIER)
    valid, msg, _ = verify_license("Pro")
    if not valid:
...
<body>
    ...
    <script>
        const amenities = ['Picture' 'Video' 'CAD';
...
document.getElementById('classifyBtn').onclick = async () => {{
    ...
    try {{
        ...
        document.getElementById('classResult').innerText = 'Classification error.';
            ...
            "QR Code": () => {{
                setInteractive(`<input id="qrText" placeholder="Enter text to encode" value="CrownStar AI"><br><div id="qrcode"></div>
<script>
